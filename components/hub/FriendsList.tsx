...
const FRIENDS: Friend[] = [
  ...
  { name: 'Жанар', initials: 'ЖА', color: 'bg-indigo-600' },
  { name: 'Ержан', initials: 'ЕР', color: 'bg-teal-600' },
  { name: 'Алина', initials: 'АЛ', color: 'bg-orange-600' },
  { name: 'Нурлан', initials: 'НУ', color: 'bg-cyan-600' },
  { name: 'Айгерим', initials: 'АЙ', color: 'bg-pink-600' },
];

export default function FriendsList() {
  return (
    <div className="w-64 border-l border-gray-800 bg-gray-950/40 p-4 h-full flex flex-col overflow-y-auto">
      <h3 className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-4 select-none">
        Онлайн друзья
      </h3>
      <div className="space-y-3.5">
        {FRIENDS.map((f, i) => (
          <div key={i} className="flex items-center justify-between group cursor-pointer hover:bg-gray-900/40 p-1.5 rounded-lg transition-colors">
            <div className="flex items-center gap-3">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold ${f.color}`}>
                {f.initials}
              </div>
